...
const invoiceSchema = z.object({
  ...
  if (data.currency !== "RSD" && !data.exchangeRate) return false;
  return true;
}, {
  ...
  return (
    <form onSubmit={form.handleSubmit(handleFormSubmit)} className="space-y-8">
          ...
          <Controller
            ...
            render={({ field }) => (
              <div className="flex items-center space-x-2 py-2">
                <Checkbox id="domesticSupply" checked={field.value} onCheckedChange={field.onChange} />
                <label
      ...
      <Button type="submit" className="w-full" disabled={isLoading}>
        {isLoading ? "..." : (submitLabel ?? t("invoices.form.submit"))}
      </Button>
    </form>
  );
};
